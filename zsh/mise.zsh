...
if [ -z "${_mise_cmd_not_found:-}" ]; then
  ...
  [ -n "$(declare -f command_not_found_handler)" ] \
    && eval "${$(declare -f command_not_found_handler)/command_not_found_handler/_command_not_found_handler}"

  function command_not_found_handler() {
    if [[ "$1" != "mise" && "$1" != "mise-"* ]] \
       && /opt/homebrew/bin/mise hook-not-found -s zsh -- "$1"; then
      _mise_hook
      "$@"
